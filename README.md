...
Apply `supabase/migrations/020_reliable_agent_flow.sql` to your existing Victor database **before deploying this web and desktop update**. It repairs missing episode ownership fields, observations and weekly reports, then installs ownership checks, deletion tombstones, ingestion revisions, session state and transactional ingestion/merge functions. Historical episodes with no recorded owner remain stored and hidden from account queries; the migration never guesses their owner. It can be rerun and does not delete existing work records. If an older copy failed with missing `observations` or `user_id`, replace the entire SQL query with this updated file and run it again.
